...
class Config:
    ...
    # Database config: defaults to SQLite for local development but supports PostgreSQL/MySQL via DATABASE_URL env var
    # Automatically corrects Render's legacy 'postgres://' prefix to 'postgresql://' for SQLAlchemy compatibility
    _db_url = os.environ.get('DATABASE_URL', 'sqlite:///grocery.db')
