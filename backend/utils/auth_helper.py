...
def encode_token(user_id, role='customer', mobile=None):
    """Generates a JWT token for the user/admin"""
    try:
        payload = {
            # 7 days expiry — better for mobile app usage (Capacitor Android/iOS)
...
def admin_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        # Allow admin, superadmin, and staff roles
        if data.get('role') not in ['admin', 'superadmin', 'staff']:
