...
from backend.database import db, Cart, CartItem, Product, Order, OrderItem, Village, Payment, InventoryLog, Notification, CustomerLocation
...
def place_order():
    ...
    whatsapp_msg_cust = (
        ...
        f"Status: Order Received. Track it here: http://localhost:5173/orders/{order.id}"
    )
    whatsapp_msg_admin = (
...
def update_order_status(order_id):
    ...
    whatsapp_msg_cust = (
        ...
        f"Track here: http://localhost:5173/orders/{order.id}"
    )
    print("\n" + "="*80)
    print(whatsapp_msg_cust)
    print("="*80 + "\n")
