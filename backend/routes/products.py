...
def upload_image():
    ...
    if file and allowed_file(file.filename):
        try:
            ...
            # 1. Main image: Resize image to fit within max size 800x800
            main_img = img.copy()
            main_img.thumbnail((800, 800), Image.Resampling.LANCZOS)
            main_img.save(main_path, 'WEBP', quality=80)
            
            # 2. Card image: Resize to fit within 300x300
            card_img = img.copy()
            card_img.thumbnail((300, 300), Image.Resampling.LANCZOS)
            card_img.save(card_path, 'WEBP', quality=75)
            
            # 3. Thumbnail image: Resize to fit within 100x100
            thumb_img = img.copy()
            ...
            image_url = f"/static/uploads/{webp_filename}"
